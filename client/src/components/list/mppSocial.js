const demoList = [
  {
    name: 'Hon. Doug Ford',
    facebook: 'https://www.facebook.com/FordNationDougFord/',
    twitter: 'https://twitter.com/fordnation?lang=en'
  },
  {
    name: 'Kathleen O. Wynne',
    facebook: 'https://www.facebook.com/WynneFans',
    twitter: 'https://twitter.com/kathleen_wynne'
  },
  {
    name: 'Andrea Horwath',
    facebook: 'https://www.facebook.com/AndreaHorwathONDP',
    twitter: 'https://twitter.com/andreahorwath'
  },
  {
    name: 'Michael Coteau',
    facebook: 'https://www.facebook.com/coteaumichael/',
    twitter: 'https://twitter.com/coteau'
  },
  {
    name: 'Hon. Sylvia Jones',
    facebook: 'https://www.facebook.com/sylviavjones',
    twitter: 'https://twitter.com/sylviajonesmpp'
  },
  {
    name: 'Mike Schreiner',
    facebook: 'https://www.facebook.com/mike.schreiner/',
    twitter: 'https://twitter.com/mikeschreiner'
  },
  {
    name: 'Gilles Bisson',
    facebook: 'https://www.facebook.com/GillesBissonONDP',
    twitter: 'https://twitter.com/bissongilles'
  },
  {
    name: 'Hon. Christine Elliott',
    facebook: 'https://www.facebook.com/ChristineElliottON/',
    twitter: 'https://twitter.com/celliottability'
  },
  {
    name: 'John Fraser',
    facebook: 'https://www.facebook.com/JohnFraserOttawaSouth',
    twitter: 'https://twitter.com/johnfraseros?lang=en'
  },
  {
    name: 'Jessica Bell',
    facebook: 'https://www.facebook.com/UniRoseJessicaBell',
    twitter: 'https://twitter.com/jessicabellto?lang=en'
  }
];

export default demoList;
